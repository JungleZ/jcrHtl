...
<%
	...
	//override with Huawei settings
    String viewerPreset="Huawei_Home_KV|VIDEO|/etc/dam/presets/viewer/Huawei_Home_KV|true";
    ValueMap kvCompVM=resource.getParent().getValueMap();
	String fileReference= kvCompVM.get("videoUrl","");
	String imageServerUrl = "";
    String videoServerUrl = "";
	if(WCMMode.fromRequest(request) == WCMMode.DISABLED){
	    imageServerUrl = kvCompVM.get("imageserverurl","");
	    videoServerUrl = kvCompVM.get("videoserverurl","");
	}else{
	    imageServerUrl=request.getContextPath() + "/is/image/";
	    //videoServerUrl= kvCompVM.get("videopreviewserverurl","");
	    videoServerUrl = kvCompVM.get("videoserverurl","");
	}
    ...
    if (renderedAsDM) {
        ...
        embedCode = buildEmbedCode(currentSession, request.getContextPath(), assetType, fileReference, getViewerPresetValue(viewerPreset),
                isCustomPreset(viewerPreset), imagePreset, stageSize, breakpoints, imageServerUrl, videoServerUrl, linkUrl, linkTarget, title, altText,
                urlModifiers, (WCMMode.fromRequest(request)), viewerInstanceId);
    } else {
    ...
    String buildViewerCode(Node viewerNode,
        ...
        if ( viewerPath != null ) {
            ...
            embedCode += "<script type=\"text/javascript\" src=\""+ viewerPath +"\"></script>\n"
                    + "<div class='dm_video_container' id=\"" + containerId + "\" data-instance-id=\"" + instanceId + "\"></div>\n"
