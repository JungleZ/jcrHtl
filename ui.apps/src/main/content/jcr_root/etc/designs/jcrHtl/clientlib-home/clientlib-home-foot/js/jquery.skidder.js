...
(function($, window, document, undefined) {
  ...
  var Skidder = {
    ...
    init: function(options, elem) {
      ...
      if (self.$slides.length > 1) {
        self.$viewport.append('<div class="skidder-prevwrapper skidder-clickwrapper visible-lg visible-md"><div class="skidder-prev skidder-clickelement"></div></div><div class="skidder-nextwrapper skidder-clickwrapper  visible-lg visible-md"><div class="skidder-next skidder-clickelement"></div></div>');
      ...
      if (self.options.scaleTo === "smallest") { 
        ...
        self.$images.each(function() {
          if ($(this).naturalHeight() > maxHeight) {
            ...
            $(this).css({
              'width'   : maxWidth,
              'height'  : Math.ceil($(this).naturalHeight() * (maxWidth / $(this).naturalWidth() )) + 2 // + 2 is lazy correction for rounding problem +300+'px'
            });
    ...
    lazyLoadSlides: function() {
      ...
      $slidestoload.each(function() {
        if (!$(this).hasClass('skidder-loaded') && $(this).attr('data-skidder-src')) {
          $(this)
            .prepend("<img class='skidder-loaded' src='" + $(this).attr('data-skidder-src') + "' title='" +  $(this).attr('data-skidder-title') + "' alt='" + $(this).attr('data-skidder-alt') + "' > <div >dddddd</div>")
            .css({
              'width'  : 'auto', 
              'height' : 'auto'
            })
    ...
    setSlideshowHeight: function(newMaxHeight) {
      ...
      self.$elem.add(self.$viewport).add(self.$wrapper).add(self.$slides).add(self.$viewport.find('.skidder-clickelement')).css('height', newMaxHeight+130+'px');
    },
  ...
  $.fn.skidder.options = {
    ...
    pagingElement   : ".skidder-pager-dot  hidden-lg",
    swiping         : true,
    leftaligned     : false,
    cycle           : true,
    jumpback        : false,
